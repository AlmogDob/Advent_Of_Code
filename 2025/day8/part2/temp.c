...
bool all_points_in_circuit_list(struct Circuit_list circuit_list, struct Points points)
{
    for (size_t i = 0; i < points.length; i++) {
        if (point_in_circuit_list(circuit_list, points.elements[i]) == -1) {
            return false;
...
int main(void)
{
    ...
    for (; !all_points_in_circuit_list(circuit_list, points);) {
        asm_dprintSIZE_T(circuit_list.length);
        min_dist = closest_points_indexes(points, min_dist, &p1_index, &p2_index);
        add_points_to_circuit_list(&circuit_list, points.elements[p1_index], points.elements[p2_index]);
        // circuit_list_print(circuit_list);
    }
    asm_dprintSIZE_T((size_t)points.elements[p1_index].x * (size_t)points.elements[p2_index].x);
